...
import { getAccounts } from '@drizzle/store/src/accounts/accountsSaga';
import { getAccountBalances } from '@drizzle/store/src/accountBalances/accountBalancesSaga';

import {
  NETWORK_IDS,
  NETWORK_MISMATCH,
} from '@drizzle/store/src/web3/constants';

export function* initializeDrizzle(action) {
  console.log('export function* initializeDrizzle(action) {');
  try {
    const { drizzle, options } = action;

    console.log('eee', drizzle, options);
    // Initialize web3 and get the current network ID.
    const { web3, writeWeb3 } = yield call(initializeWeb3, options.web3);
    console.log('response', web3, writeWeb3);
    drizzle.web3 = web3;
    drizzle.writeWeb3 = writeWeb3;
