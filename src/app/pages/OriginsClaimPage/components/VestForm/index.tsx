...
interface IVestFormProps {
  address: string;
  className?: string;
}
export const VestForm: React.FC<IVestFormProps> = ({ className, address }) => {
  ...
  const handleSubmit = useCallback(() => {
    send(
      ...
      {
        type: TxType.LOCKED_FUND_CREATE_STAKE,
      },
    );
  }, [address, send]);
    ...
    >
      ...
      <div className="tw-px-8 tw-mt-6 tw-flex-1 tw-flex tw-flex-col tw-justify-center">
        ...
        <div className={!rewardsLocked ? 'tw-mt-10' : undefined}>
          ...
          {!rewardsLocked && (
            <Button
              disabled={parseFloat(balance) === 0 || !balance || rewardsLocked}
              onClick={handleSubmit}
              className="tw-w-full tw-mb-4"
              text={t(translations.originsClaim.vestForm.cta)}
            />
          )}
