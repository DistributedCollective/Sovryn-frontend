...
import { useEstimateZeroToSov } from './useEstimateZeroToSov';

export const useDollarValueZero = (weiAmount: string) => {
  const asset = Asset.SOV;
  const sovDollars = useCachedAssetPrice(asset, Asset.USDT);
  const sovWeiAmount = useEstimateZeroToSov(weiAmount);
